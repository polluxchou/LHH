...
export default async function MembersPage({ searchParams }: { searchParams: Promise<{ space?: string }> }) {
  ...
  if (mine.length === 0) redirect("/no-space");
  const target = mine.find((m) => m.space.id === space) ?? mine[0];
  return (
    <main className="account-page">
      <div className="account-page-head">
        <Link href={`/?space=${target.space.id}`} className="link-btn">← Workbench</Link>
        <h1>{target.space.name}</h1>
      </div>
      <MemberPanel spaceId={target.space.id} actorRole={target.role} isOwner={target.isOwner} locale="en" />
    </main>
  );
}
